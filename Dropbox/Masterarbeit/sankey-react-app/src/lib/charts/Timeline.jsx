        ...
        data.events.forEach(function (d) {
            d.events.forEach(function (e, i) {
                ...
                if ("endNumberOfDaysSinceDiagnosis" in e) {
                    dates.push(
                        <rect opacity={opacity} key={data.type + d.patient + "" + i} transform={transform} y="-5"
                              height="10" width={x(e.endNumberOfDaysSinceDiagnosis - e.startNumberOfDaysSinceDiagnosis)}
                              fill={data.color}/>
                    )
                }
                else {
                    dates.push(
                        <circle opacity={opacity} key={data.type + d.patient + "" + i} transform={transform} r="5"
                                fill={data.color}/>
                    )
                }
            })
